...
todos:
  - id: phase-0-prisma-auth
    content: "Phase 0: Prisma + PostgreSQL setup, Better Auth (cookie sessions, register/login), server structure, app shell, auth pages (React Hook Form + zodResolver), API client, admin role + pages (tasks 0.1 -- 0.7); admin /users UX + invite form aligned with exercise library (debounced search, shared ExerciseLibraryPagination, RHF+Zod adminInviteFormSchema, shadcn Select for role)"
    status: completed
  - id: phase-1-exercise-lib
    content: "Phase 1: Exercise Library -- schema (Exercise, ExerciseFolder, ExerciseImage, ExerciseLayer, DropdownCategory, DropdownOption), CRUD API, image uploads (Cloudinary hybrid temp flow), progression linking, dynamic dropdowns, extended fields (equipment/spinalMovement/chainType/jointLoading as String[], progressionNotes/regressionNotes), layer system with explicit isFinisher boolean, Fancybox lightbox, library UI with search/filter/folders, **exercise-form-multistep** (5-step wizard for library create/edit; API-backed dropdowns, folders, images, step validation on Continue, free tab nav, onInvalid jump-to-first-errored-step) + **exercise-form** (single-page; class-plan embed only) via React Hook Form + Zod (`useFieldArray` for layers, `Controller` for selects + `BulletTextarea`), shared client schemas in `client/src/lib/validation/`, **bullet-textarea** (Enter/Shift+Enter + Add •; toolbarEndSlot works in both bulletsEnabled and plain modes) on long-text fields and every layer row, **exercise-pre-text** on detail for formatted display, **exercise-library-pagination** on `/exercises` wired to **GET /api/exercises** optional `page`/`limit` via **getExerciseListPage** + **useExerciseList** (unpaginated **getExercises** for pickers); same controlled component on `/admin/users` (tasks 1.1 -- 1.8)"
    status: completed
...
- **Server** ([server/src/app.ts](server/src/app.ts)): Express 4 with Better Auth mounted at `/api/auth/`*, health endpoint, CORS with credentials, Prisma 7 + PostgreSQL, modular feature structure under `server/src/modules/`. Modules: `admin/` (invitation, settings, stats), `exercises/` (full CRUD + folders + progression + images + reorder; `**GET /api/exercises`** returns a paginated object `{ exercises, total, page, limit }` when query `**page**` is set, otherwise a JSON **array** for backward-compatible callers such as class-plan pickers), `class-plans/` (template + folder CRUD, nested sections/exercises, duplicate; `**GET /api/class-plans`** returns `{ data, total, page, limit }`), `dropdowns/` (dynamic option API), `uploads/` (temp image upload/delete). Route mounts: `/api/admin`, `/api/uploads`, `/api/exercises`, `/api/exercise-folders`, `/api/class-plans`, `/api/class-plan-folders`, `/api/dropdowns`, plus public helpers (`/api/signup-status`, `/api/invite/verify`).
- **Client** ([client/src/app/(dashboard)/page.tsx](client/src/app/(dashboard)/page.tsx)): Next.js 16 App Router with dashboard layout, auth pages (`/login`, `/register` with **React Hook Form** + `**zodResolver`** from `@hookform/resolvers/zod` and schemas in `[client/src/lib/validation/auth-schemas.ts](client/src/lib/validation/auth-schemas.ts)`), Better Auth React client, **sidebar-only shell** (mobile drawer + fixed desktop rail): main content beside `[AppLayout](client/src/components/layout/app-layout.tsx)` with no top bar; **account menu at the bottom of `[Sidebar](client/src/components/layout/sidebar.tsx)`** — avatar + name/email trigger, dropdown (`side="top"`) with profile summary and **Log out** (same `useAuth().logout` + `/login` redirect as the former top bar). Role-aware admin navigation remains in the sidebar. Full Exercise Library UI (list/grid views, create, edit, detail with Fancybox image lightbox, layers with isFinisher badge styling, dropdown-driven fields, progression chain viewer). **Exercise create/edit** uses `**exercise-form-multistep`** (5-step wizard: Basics, Equipment, Layers & cues, Movement analysis, Progression; API-backed dropdown options, folders, images with dropzone + reorder + Fancybox, per-step `trigger(STEP_FIELD_GROUPS)` validation on Continue, free step-tab navigation, `SubmitErrorHandler` toast + jump to first errored step, native submit blocked until last step, final save is `type="button"` + explicit `submitForm()`); `**exercise-form**` (single-page) retained for **class-plan embed** flows only (`embedInClassPlan` + callbacks). `/exercises/multistep` redirects to `/exercises/new`. Sidebar "Multistep form" nav item removed. **Admin `/admin/users`**: UX aligned with the exercise library — single header card (title + live stats + **Invite user**), debounced email search with clear control, filtered-empty vs directory-empty states (same patterns as `exercise-list`), table + `**[ExerciseLibraryPagination](client/src/components/exercises/exercise-library-pagination.tsx)`** in **controlled** mode (first / prev / numbered pages / next / last — same chrome as the exercise / class-plan library pagination). Invite dialog: **React Hook Form** + `**zodResolver`** + `[adminInviteFormSchema](client/src/lib/validation/auth-schemas.ts)` (email + role), shadcn `**Select**` for role (trigger/content/item classes match `exercise-form`), field errors with `aria-invalid` + destructive helper text like the exercise form. `/admin/settings` unchanged at a high level. **Class Plans** routes `**/class-plans`** (template library) and `**/class-plans/[id]**` (detail / section builder + exercise picker). Service modules: `exercise-api` (`**getExercises**`, `**getExerciseListPage**`, `**EXERCISE_LIBRARY_PAGE_SIZE**`), `class-plan-api` (`**listClassPlans**`, `**CLASS_PLAN_LIBRARY_PAGE_SIZE**`), `admin-api`, `dropdown-api`. Custom hooks: `use-debounce`, `use-dropdown-options`, `use-fancybox`, exercise hooks (`use-exercise-folders`, `use-exercise-library`, `use-exercise-list`, `use-exercise-search`), class-plan hooks (`use-class-plan-folders`, `use-class-plan-library`, `use-class-plan-list`). Exercise components: `exercise-form-multistep` (primary library create/edit wizard — optional `exercise?: Exercise` prop; RHF + Zod via `[exercise-form-schema.ts](client/src/lib/validation/exercise-form-schema.ts)`, `useFieldArray` for layers, `Controller` for shadcn `Select` + `BulletTextarea`, `useDropdownOptions` for all catalog selects, folder + image management, `STEP_FIELD_GROUPS` per-step `trigger` on Continue, `SubmitErrorHandler` jumps to first step with errors; progression `Select` **commented out** — restore block + `getExercises`/watch/memos documented in comment), `exercise-form` (single-page fallback for class-plan embed; same RHF+Zod stack, progression `Select` also commented out), `bullet-textarea` (reusable long-text + inline `•` authoring; `toolbarEndSlot` renders in **both** `bulletsEnabled` and plain `bulletsEnabled={false}` modes so layer remove buttons always appear), `exercise-pre-text` (detail page: preserves line breaks and indents lines that start with `•`), `exercise-list` (with filtered-empty state + "Clear filters" action), `exercise-card`, `exercise-search`, `exercise-library-header` (unified: title + stats + "New folder" / "New exercise" actions + search bar + folder Select dropdown with inline Rename/Delete buttons for the selected folder), `**exercise-library-pagination`** (always under `**/exercises**` and `**/class-plans**` with `**ariaLabel**`; **controlled** props + `**loading`**; **controlled** on `**/admin/users`**), `folder-dialog`, `progression-chain-viewer`. **Removed**: `top-bar.tsx` (account + **Log out** moved to bottom of `Sidebar`), `exercise-folder-sidebar` (replaced by folder Select dropdown in header), `exercise-library-toolbar` (merged into `exercise-library-header`).
...
## Notes: list API pagination (exercises vs class plans)

- `**GET /api/exercises` — dual response (by design)**  
  - **With `page` (and optional `limit`)** → `**{ exercises, total, page, limit }`**. Used by the exercise **library** page: `**client/src/hooks/exercises/use-exercise-list.ts`** (always passes `**limit: EXERCISE_LIBRARY_PAGE_SIZE**` with `**page**`), `**exerciseApi.getExerciseListPage**`.  
  - **Without `page`** → JSON **array** of exercises. Used by the class-plan **exercise picker** so search/filter runs over the full matching set in one response: `**client/src/components/class-plans/exercise-picker-dialog.tsx`**, `**exerciseApi.getExercises**`.  
  - **Rationale**: avoid stitching pages in dialogs; keep a single route for list + filters. If the multistep **progression-of** parent `Select` is re-enabled, it would use `**getExercises`** again (comment in `**exercise-form-multistep.tsx**`).
- `**GET /api/class-plans` — single shape**  
Always `**{ data, total, page, limit }`**. `**client/src/hooks/class-plans/use-class-plan-list.ts**`, `**classPlanApi.listClassPlans**`. No dual response; no equivalent "load every template row" consumer on this list API.
- **Naming contrast**: paginated exercise payload uses `**exercises`**; class plans use `**data**`. Both include `**total**`, `**page**`, `**limit**`.

See also: `[.cursor/rules/project.mdc](.cursor/rules/project.mdc)` (Backend **Pagination design note — exercise list dual response** and `**exerciseApi`** mapping).
...
- **Client forms (May 2026)**: `**react-hook-form`** + `**zodResolver**` (`@hookform/resolvers/zod`) with **Zod 4** schemas under `client/src/lib/validation/` — login, register, exercise create/edit, and **admin invite** (`[adminInviteFormSchema](client/src/lib/validation/auth-schemas.ts)` on `/admin/users`). Prefer aligning client rules with server Zod (`exercise.validation.ts`, etc.) where practical.
...
- **0.4 -- App shell and layout** ✓
  - Replace default page with app layout: sidebar navigation + main content area (no top bar; mobile hamburger + `pt-14` on main so content clears the menu button)
  - Reusable layout components in `client/src/components/layout/`: `[AppLayout](client/src/components/layout/app-layout.tsx)` (auth gate + flex shell), `[Sidebar](client/src/components/layout/sidebar.tsx)` (nav links, scrollable middle, account dropdown + **Log out** pinned to bottom)
...
- **0.5 -- Auth pages and client-side auth state** ✓
  ...
  - Role-aware sidebar with admin navigation section (Admin Dashboard, User Management, Seed Exercises, Platform Stats, Settings); signed-in user menu and **Log out** live at the bottom of the sidebar (dropdown), not in a separate header
- **0.7 -- Admin pages (UI)** *(partially complete — users + settings production-quality; seed/stats deferred)*
  - `/admin` dashboard with key stats cards ✓
  - `/admin/users` user management: table with invite, activate/deactivate, role change ✓; **UI/UX (May 2026)** — same patterns as Exercise Library (unified header card, debounced search + clear, empty states, shared `**[ExerciseLibraryPagination](client/src/components/exercises/exercise-library-pagination.tsx)`** in controlled mode); invite dialog **RHF + `zodResolver` + `adminInviteFormSchema`** (email + role) and shadcn `**Select**` for role styled like `**exercise-form**` selects; field-level validation display (`aria-invalid`, destructive messages)
...
- **1.2 -- Exercise CRUD API** ✓
  - `POST/GET /api/exercises`, `GET/PATCH/DELETE /api/exercises/:id` — `**GET /api/exercises*`* supports optional `**page`/`limit**` (validated query); with `**page**`, response is `**{ exercises, total, page, limit }**`; without `**page**`, response is the full exercise **array** (picker compatibility)
...
- **1.5 -- Exercise Library UI** ✓
  - `/exercises` page: unified `exercise-library-header` card (title + live stats from `**listTotalCount ?? totalExerciseCount`** — "N exercises · M folders" / "Showing X of Y exercises" when filtered, "New folder" + "New exercise" action buttons, search bar via `exercise-search`, folder Select dropdown with per-folder counts + inline Rename/Delete buttons when a folder is selected), `exercise-list` grid with `exercise-card` components. Search placeholder: "Search by exercise name or description…" (matches backend `name` + `description` `contains` query). Filtered-empty state uses `**listTotalCount === 0**` with active filters while the account still has exercises. `**exercise-library-pagination**`: always rendered under the grid (`**useExerciseList**` + `**getExerciseListPage**` → `**GET /api/exercises?page=&limit=**`; default page size `**EXERCISE_LIBRARY_PAGE_SIZE**`); pass `**page**`, `**totalPages**`, `**onPageChange**`, `**loading**`, `**ariaLabel**`. Unpaginated `**getExercises**` remains for class-plan exercise picker and similar callers. Same **controlled** mode on `**/admin/users`**.
  - `/exercises/new` and `/exercises/[id]/edit` use `**exercise-form-multistep**` (5-step wizard: Basics → Equipment → Layers & cues → Movement analysis → Progression); `/exercises/multistep` redirects to `/exercises/new`. The multistep form accepts an optional `exercise` prop for edit mode and uses **React Hook Form** + `**zodResolver`** (`[exercise-form-schema.ts](client/src/lib/validation/exercise-form-schema.ts)`) — `useFieldArray` for dynamic layers, `Controller` for shadcn `Select` and `BulletTextarea`, `useDropdownOptions` for all catalog selects, `register` / `setValue` / `getValues` / `useWatch` for the rest. Step navigation: `trigger(STEP_FIELD_GROUPS[step])` on Continue, free step-tab clicks without validation, `SubmitErrorHandler` toast + auto-jump to first step with errors on invalid final save. Native `<form onSubmit>` blocks submission until last step; final button is `type="button"` calling `submitForm()`. Image state (dropzone, reorder, temp IDs) remains outside RHF; react-dropzone (max 3, 5 MB), HTML5 drag-to-sort for both temp and saved images, remove button for both temp (API delete) and saved images. `**exercise-form**` (single-page) remains for class-plan embed flows only
  ...
  - `**bullet-textarea**` ([client/src/components/exercises/bullet-textarea.tsx](client/src/components/exercises/bullet-textarea.tsx)): reusable controlled textarea — `value` + `onValueChange`; **Enter** → insert `\n•` , **Shift+Enter** → newline without new bullet; **Add •** inserts `•`  at caret only when selection is collapsed, caret is at the start of the current line, and that line does not already begin with `•` ; optional `**label`** (e.g. shadcn `Label`) and `**toolbarEndSlot**` (e.g. layer remove) on the same toolbar row as **Add •** (`data-bullet-textarea-toolbar` keeps focus/blur behavior correct); `**bulletsEnabled={false}`** renders a plain shadcn `Textarea` but still shows `label` + `toolbarEndSlot` in a top row when either is provided (ensures layer remove buttons appear in non-bulleted contexts). Wired in `**exercise-form-multistep**` and `**exercise-form**` (via `Controller`) for Description, Starting position, **all** dynamic layer contents, Cues / notes, progression notes, and regression notes (each layer: step title + finisher UI in `label`, remove control in `toolbarEndSlot` when `index > 0` and more than one layer row)
  - Folder management (create, rename, delete) via `folder-dialog` triggered from header buttons (no separate sidebar)
  - **Removed components**: `exercise-folder-sidebar` (replaced by folder Select dropdown in `exercise-library-header`), `exercise-library-toolbar` (merged into `exercise-library-header`)
  - Custom hooks: `use-exercise-folders`, `use-exercise-library`, `use-exercise-list`, `use-exercise-search`, `use-debounce`
  - Service module: `exercise-api` (CRUD, `**getExerciseListPage`** / `**getExercises**`, temp uploads, image reorder, progression chain, save-to-library)
...
- **1.8 -- Exercise Layer System** ✓
  ...
  - Both forms render dynamic layer rows (add/remove) numbered sequentially (`Layer 1`, `Layer 2`, …) via `**useFieldArray`** (`layers`); `**buildExerciseFormDefaults**` + `reset` when switching exercises by `id`
    ...
    - **Layer remove rule**: remove button shown only when `index > 0 && layerFields.length > 1` — Layer 1 never has a remove button (schema requires ≥ 1 layer)
  ...
  - **Form UX**: every appended layer row uses `**BulletTextarea`** with bullets enabled (same Enter / Shift+Enter / **Add •** behavior as Layer 1) in both `exercise-form-multistep` and `exercise-form`
...
> **Status (May 2026)**: **2.1 / 2.2 largely complete** — Prisma includes `ClassPlanFolder`, `ClassPlanTemplate` (with `classType`, `classStyle`, `durationMinutes`, `folderId`, `tags`), `PlanSection`, `PlanSectionExercise`, and `Exercise.savedToLibrary`. `**server/src/modules/class-plans/`** implements folder + template CRUD, nested sections/exercises, duplicate, and paginated listing. `**PATCH /api/exercises/:id/save-to-library**` is live. **2.3 UI** — `**/class-plans`** library (filters, `**useClassPlanList**` + `**ExerciseLibraryPagination**`) and `**/class-plans/[id]**` planner (sections, exercise picker library/create, programming summary) are implemented; **drag-and-drop reorder** for sections/exercises and **quick-schedule / "Schedule" Flow 2** from the plan doc are still **outstanding** Phase 2 items.
...
- **2.2 -- Template CRUD API** *(implemented in `server/src/modules/class-plans/` — filenames: `class-plan.routes.ts`, `class-plan.service.ts`, `class-plan.validation.ts`, `class-plan-folder.routes.ts` + services)*
  - Folder endpoints:
  ...
  - Template endpoints:
    ...
    - `GET /api/class-plans` — list (filter: folderId, classType, classStyle, tags, search; filter `deletedAt IS NULL`; response `**{ data, total, page, limit }`** with `**page`/`limit**` query params, defaults in Zod schema)
...
- **2.3 -- Class Planner UI (template builder)**
  - `/class-plans` listing page:
    - Folder filter in the page header (same UX pattern as the Exercise Library: folder `Select` + counts / rename / delete in the header card — **not** a separate `exercise-folder-sidebar`-style rail; that component was removed from exercises)
    - Template grid/list with search, filter by classType, classStyle, tags; **paginated** via `**listClassPlans`** + `**ExerciseLibraryPagination**` (`**useClassPlanList**`, `**CLASS_PLAN_LIBRARY_PAGE_SIZE**`, bar always visible with `**ariaLabel**`)
...
- **6.2 -- Dashboard UI**
  ...
  - Calendar mini-view (month at a glance with differentiate color for today, schedule classes, and done classes)
- **6.3 -- Reporting page**
