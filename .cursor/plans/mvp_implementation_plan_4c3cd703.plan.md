...
- **1.1 -- Prisma schema: Exercise, ExerciseFolder, ExerciseImage, ExerciseLayer ✓**
  ...
  - `Exercise`: id, name, description, startingPosition, orientation, directionFaced, movementType, springs, equipment, machineSetup, transitionCues, cueing, spinalMovement (string[], multiselect), chainType, jointLoading, tags (string[]), folderId, instructorId, progressionOfId (self-relation), createdAt, updatedAt, deletedAt
...
- **1.7 -- Extended Exercise Fields ✓**
  - 11 new optional fields on Exercise model (orientation through jointLoading); `spinalMovement` is `String[]` (multiselect array, default `[]`) while the rest are optional single strings
  - Dropdown-driven selects in ExerciseForm for most fields; `spinalMovement` uses checkboxes for multiselect
  - Displayed on exercise detail page in organized sections (Setup card, Movement Analysis card); spinal movement values rendered as badges
