...
- **1.5 -- Exercise Library UI** ✓
  - `/exercises` page: unified `exercise-library-header` card (title + live stats "N exercises · M folders" / "Showing X of Y exercises" when filtered, "New folder" + "New exercise" action buttons, search bar via `exercise-search`, folder Select dropdown with per-folder counts + inline Rename/Delete buttons when a folder is selected), `exercise-list` grid with `exercise-card` components. Search placeholder: "Search by exercise name or description…" (matches backend `name` + `description` `contains` query). Filtered-empty state with "Clear filters" action when folder/search yields zero results but library is non-empty. **`exercise-library-pagination`**: rendered under the grid with **placeholder** controls (non-functional page bar until exercise list API supports offset/limit paging); the same component supports a **controlled** mode (`page`, `totalPages`, `onPageChange`, `loading`, optional `ariaLabel`) — used on **`/admin/users`** so admin pagination matches this UI.
